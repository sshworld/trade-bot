...
### TP — 풀물량 단일 TP 8% (2026-06-08 회의록, 분할/트레일 폐기)
| 단계 | 마진 수익% | BTC 이동% (@5x) | 물량 | 후속 |
|------|:---:|:---:|:---:|:---:|
| TP | **8%** | 1.6% | **100%** | 한 번에 전량 익절 (러너 없음) |

- 근거: 백테스트(실거래 63건 진입 고정·청산규칙만) — 분할+트레일 net 61.9%/MDD10 vs 풀 TP8% net 120%/MDD19. 분할은 승리의 70%가 본전 트레일("찔끔")로 깔리는 구조 결함.
- 순수 1위 TP10%(net 134)는 생존편향 과대평가·적중35% 위험 → PF 동률(2.51)·적중40%인 **TP8% 채택** (Buffett·Dimon 합의).
- 검증 게이트: +30 실거래 AND $196 회복 후에만 TP 상향 재검토.
- ~~분할 TP1/2/3 (3/6/10% @ 50/30/20)~~ 폐기. 트레일링(아래)은 단일 TP라 비활성.

### SL — 잔고 2% 고정 손실
- 전량 1건 청산 (분할 SL 없음)
- 최소 SL 거리: **0.5%** (2026-06-08: 0.3→0.5, 노이즈 스톱 완화)
- 물타기 시 평단 기준 재계산
