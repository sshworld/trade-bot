...
| 동시 포지션 | 1개 |
| SL 리스크 | **증거금 2% 손실 = 가격 0.4% (5x)** (2026-06-09) |
...
### TP — 풀물량 단일 TP 마진 2% (가격 0.4% @5x) (2026-06-09, 0.4% 대칭 스캘프)
| 단계 | 마진 수익% | BTC 이동% (@5x) | 물량 | 후속 |
|------|:---:|:---:|:---:|:---:|
| TP | **2%** | 0.4% | **100%** | 한 번에 전량 익절 (러너 없음) |

- SL/TP = 증거금 2% (가격 0.4%) 대칭, R:R 1:1.
- 손익분기 승률 ≈ 60% (taker 왕복 수수료 ≈0.4% margin 감안). 고승률 단타 전제.
- 검증 게이트: +N 실거래 후 승률·real_profit 모니터링 → 재검토.
- ~~풀물량 TP8%(2026-06-08)~~ → 0.4% 스캘프로 교체 (2026-06-09). ~~분할 TP1/2/3~~ 폐기 유지. 트레일링(아래)은 단일 TP라 비활성.

### SL — 증거금 2% 손실(가격 0.4%) 대칭, 평단 기준
- 전량 1건 청산 (분할 SL 없음)
- 최소 SL 거리: **0.0%** (floor 제거, 2026-06-09)
- 물타기 시 평단 기준 재계산 + **추매 체결마다 SL·TP 양쪽 Binance 재배치**
...
### 진입 (물타기, ATR 기반 offset, 2026-06-09 간격 축소)
- 50%/30%/20% @ [0, ATR×1.0, ATR×2.0] 역행 (floor 0%/0.1%/0.2%, 캡 0%/0.2%/0.35%) — 0.4% SL 안쪽으로 축소(깊은 추매가 SL에 막히지 않게)
...
- [x] 테스트 DB 격리 (conftest autouse) — pytest 실DB 오염 회귀 방지 (2026-06-08)
